...
class Converter(commands.Cog):
    ...
    async def text_to_ascii(self, ctx, *, text: str):
        await ctx.send(
            "Text To ASCII: ```" + " ".join(str(ord(char)) for char in text) + "```"
        )
    ...
    async def text_to_binary(self, ctx, *, text: str):
        await ctx.send(
            "Text To Binary: ```"
            + " ".join(bin(ord(char))[2:] for char in text)
            + "```"
        )
    ...
    async def text_to_hex(self, ctx, *, text: str):
        await ctx.send(
            "Text To Hex: ```" + " ".join(hex(ord(char))[2:] for char in text) + "```"
        )
    ...
    async def octal_to_text(self, ctx, *, octal: str):
        await ctx.send(
            "Octal To Text: ```"
            + "".join(
                chr(int(octal_number, 8)) for octal_number in octal.split() + "```"
            )
        )
    ...
    async def morse_to_text(self, ctx, *, morse: str):
        ...
        await ctx.send("Morse To Text: ```" + msg + "```")
